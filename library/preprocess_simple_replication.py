...
def run():
    ...
    data = pandas.read_csv('raw_data/Nurses_And_Robots_replication_December+7,+2021_08.37.csv')
    ...
    # %%
    with pandas.ExcelWriter('data/preprocessed_simple_replication.xls') as writer:
        demo_data.to_excel(writer, sheet_name='demo_data')
        accuracy.to_excel(writer, sheet_name='accuracy')

    # %% output
    data_table = rating_data
    with pandas.ExcelWriter('data/data_table_simple_replication.xls') as writer:
